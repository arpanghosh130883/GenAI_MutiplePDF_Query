...
def generate_llama_response(job_description_text, resume_text, jd_match, position_match, education_match, stability_percentage):

    ### LLama2 model
    llm=CTransformers(model='hug_models/llama-2-7b-chat.ggmlv3.q8_0.bin',
                      model_type='llama',
    ...
    prompt = prompt_template.format(
        ...
        jd_match=jd_match,
        position_match=str(position_match),
        education=str(education_match),
        stability=str(stability_percentage)
    )
...
if submit:
    if uploaded_resume is not None and uploaded_job_description is not None:
        ...
        response = generate_llama_response(job_description_text, resume_text, jd_match, position_match, education_match, stability_percentage)
        print(response)
